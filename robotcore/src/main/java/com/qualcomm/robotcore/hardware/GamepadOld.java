...
public class GamepadOld implements RobocolParsable
{
    ...
    private final GamepadOldCallback c;
    ...
    public GamepadOld(final GamepadOldCallback callback) {
    ...
    public void copy(final GamepadOld gamepad) throws RobotCoreException {
        this.fromByteArray(gamepad.toByteArray());
    }

    public void reset() {
        try {
            this.copy(new GamepadOld());
        }
        catch (RobotCoreException ex) {
            RobotLog.e("GamepadOld library in an invalid state");
            throw new IllegalStateException("GamepadOld library in an invalid state");
        }
    }
    ...
    public static void enableWhitelistFilter(final int vendorId, final int productId) {
        if (GamepadOld.e == null) {
            GamepadOld.e = new HashSet<a>();
        }
        GamepadOld.e.add(new a(vendorId, productId));
    }
    ...
    @TargetApi(19)
    public static synchronized boolean isGamepadOldDevice(final int deviceId) {
        if (GamepadOld.d.contains(deviceId)) {
            return true;
        }
        GamepadOld.d = new HashSet<Integer>();
        for (final int n : InputDevice.getDeviceIds()) {
            ...
            if ((sources & 0x401) == 0x401 || (sources & 0x1000010) == 0x1000010) {
                if (Build.VERSION.SDK_INT >= 19) {
                    if (GamepadOld.e == null || GamepadOld.e.contains(new a(device.getVendorId(), device.getProductId()))) {
                        GamepadOld.d.add(n);
                    }
                }
                else {
        ...
        return GamepadOld.d.contains(deviceId);
    }
    ...
    public interface GamepadOldCallback
    {
        void gamepadChanged(final GamepadOld p0);
    }
}