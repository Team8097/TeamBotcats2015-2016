...
public class ReadXMLFileHandler
{
    ...
    public List<ControllerConfiguration> parse(final InputStream is) throws RobotCoreException {
        this.m = null;
        try {
            final XmlPullParserFactory instance = XmlPullParserFactory.newInstance();
            instance.setNamespaceAware(true);
            (this.m = instance.newPullParser()).setInput(is, (String)null);
    ...
    private ControllerConfiguration a() throws IOException, XmlPullParserException, RobotCoreException {
        ...
        while (i != 1) {
            ...
            if (i == 2) {
                ...
                if (s.equalsIgnoreCase(DeviceConfiguration.ConfigurationType.I2C_DEVICE.toString()) || s.equalsIgnoreCase(DeviceConfiguration.ConfigurationType.IR_SEEKER_V3.toString()) || s.equalsIgnoreCase(DeviceConfiguration.ConfigurationType.ADAFRUIT_COLOR_SENSOR.toString()) || s.equalsIgnoreCase(DeviceConfiguration.ConfigurationType.COLOR_SENSOR.toString()) || s.equalsIgnoreCase(DeviceConfiguration.ConfigurationType.GYRO.toString())) {
    ...
    private ArrayList<DeviceConfiguration> a(final int n, final DeviceConfiguration.ConfigurationType type) {
        final ArrayList<DeviceConfiguration> list = new ArrayList<DeviceConfiguration>();
        for (int i = 0; i < n; ++i) {
            if (type == DeviceConfiguration.ConfigurationType.SERVO) {
                list.add(new ServoConfiguration(i + 1, "NO DEVICE ATTACHED", false));
            }
            else if (type == DeviceConfiguration.ConfigurationType.MOTOR) {
                list.add(new MotorConfiguration(i + 1, "NO DEVICE ATTACHED", false));
            }
            else {
                list.add(new DeviceConfiguration(i, type, "NO DEVICE ATTACHED", false));
            }
        }
